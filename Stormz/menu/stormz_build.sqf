...
_canBuild = [_pos, _this select 0, true] call dze_buildChecks;
if (_canBuild select 0) then {
	...
	while {_isOk} do {
		...
		if(_location1 distance _location2 > MaxMoveDistance) exitWith {
			_isOk = false;
			_cancel = true;
			_reason = format[localize "STR_EPOCH_BUILD_FAIL_MOVED",MaxMoveDistance];
			detach _object;
			deleteVehicle _object;
			if (_VectorWorkAround) then {
				detach _object2;
		...
		if(_location1 distance _objectHelperPos > MaxMoveDistance) exitWith {
			_isOk = false;
			_cancel = true;
			_reason = format[localize "STR_EPOCH_BUILD_FAIL_TOO_FAR",MaxMoveDistance];
			detach _object;
			deleteVehicle _object;
			if (_VectorWorkAround) then {
				detach _object2;
		...
		if(abs(_objHDiff) > MaxHeightDistance) exitWith {
			_isOk = false;
			_cancel = true;
			_reason = format[localize "STR_EPOCH_BUILD_FAIL_HEIGHT",MaxHeightDistance];
			detach _object;
			deleteVehicle _object;
			if (_VectorWorkAround) then {
				detach _object2;
	...
	if(!_cancel) then {
		...
		if (_proceed) then {
	        ...
	        if (count _array > 1) then {
	            {
		            _num_removed = [player,_x select 0,_x select 1] call BIS_fnc_invRemove; //remove item's magazine from inventory
					_number = _x select 1;
		        } forEach _array;
	        }else{
		        _num_removed = [player,((_array select 0) select 0),((_array select 0) select 1)] call BIS_fnc_invRemove; //remove item's magazine from inventory
				_number = ((_array select 0) select 1);
	        };
			
			if(_num_removed == _number) then {
					...
					if (_tmpbuilt isKindOf "Land_Fire_DZ") then { //if campfire, then spawn, but do not publish to database
						...
						if (DZE_permanentPlot) then {
							...
							if (_canBuild select 1) then {
								...
								//diag_log format ["publicvariable canbuild select 1: %1",PVDZ_obj_Publish];
							} else {
								PVDZ_obj_Publish = [dayz_characterID,_tmpbuilt,[_dir,_location,dayz_playerUID,_vector],[],player,dayz_authKey];
								//diag_log format ["publicvariable: %1",PVDZ_obj_Publish];
							};
						} else {
